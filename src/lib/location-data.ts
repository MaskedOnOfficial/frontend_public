// India location data: country → states → districts
// Districts listed are the major/official ones per state.
    ...
    states: [
      ...
      },
      {
        ...
        districts: [
          ...
          "Bidar", "Chamarajanagara", "Chikkaballapur", "Chikkamagaluru",
      ...
      },
      {
        ...
        districts: [
          ...
          "Mandsaur", "Morena", "Narsimhapur", "Neemuch", "Niwari",
      ...
      },
      {
        ...
        districts: [
          ...
          "Khairthal-Tijara", "Kotputli-Behror", "Kota", "Nagaur",
...
export function getStatesForCountry(country: string): string[] {
  const found = LOCATION_DATA.find((c) => c.country === country);
...
export function getDistrictsForState(country: string, state: string): string[] {
  const found = LOCATION_DATA.find((c) => c.country === country);
  if (!found) return [];
  const stateData = found.states.find((s) => s.name === state);
  return stateData ? stateData.districts : [];
}

export const COUNTRIES = LOCATION_DATA.map((c) => c.country);
