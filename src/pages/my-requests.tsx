...
import {
  ...
  ArrowUpRight, ArrowDownLeft, UserCheck, UserX, Users,
} from "lucide-react";
...
interface IncomingCardProps {
  req: PartyRequest;
  index: number;
  onAction: (req: PartyRequest, status: "approved" | "rejected") => void;
    ...
    >
      ...
      <div className="pl-5 pr-4 pt-4 pb-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2.5 mt-1.5 text-[10px] text-text-dim flex-wrap">
              ...
              {req.parties_attended !== undefined && (
                <span>{req.parties_attended} events attended</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-border/50">
          <Link to={`/parties/${req.party_id}`} className="text-xs font-bold text-primary hover:text-accent transition truncate block">
            {req.party_title}
          </Link>
...
export default function MyRequestsPage() {
  ...
  const [hostRequests, setHostRequests] = useState<PartyRequest[]>([]);
  ...
  const handleAction = useCallback(async (req: PartyRequest, status: "approved" | "rejected") => {
    setActionError("");
    setActingId(req.id);
    try {
      await api.patch(`/parties/${req.party_id}/requests/${req.id}`, { status });
  ...
  const hostFiltered = useMemo(() => {
    if (hostTab === "all") return hostRequests;
    return hostRequests.filter((r) => r.status === hostTab);
  }, [hostRequests, hostTab]);
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 md:py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-text-dim text-sm mt-0.5">Track every request — sent and received.</p>
          </div>
          <motion.button
            whileTap={{ scale: 0.92 }}
        ...
        <motion.div
          ...
          className="grid grid-cols-2 gap-2 mb-5 p-1 rounded-2xl bg-surface border border-border"
        >
          <motion.button
            whileTap={{ scale: 0.96 }}
              ...
              <AnimatePresence mode="wait">
                {hostLoading ? (
                  ...
                  >
                    ...
                    <p className="text-text font-bold text-base mb-1">
                      {hostTab === "all" ? "No incoming requests" : `No ${hostTab} requests`}
                    </p>
                    <p className="text-text-muted text-sm">
                      {hostTab === "all"
                        ? "When someone requests to join your party, they'll appear here."
                        : `No requests with ${hostTab} status right now.`}
                    </p>
                  </motion.div>
                ) : (
